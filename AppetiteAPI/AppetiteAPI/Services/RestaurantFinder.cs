...
namespace AppetiteAPI.Services
{
    public class RestaurantFinder : IRestaurantFinder
    {
        ...
        public List<RestaurantModel> GetCloseRestaurants(RestaurantFinderModel model)
        {
            ...
            {
                if (new GeoCoordinate(model.Coordinate.Latitude, model.Coordinate.Longitude)
                    .GetDistanceTo(new GeoCoordinate(restaurant.Adress.Latidude, restaurant.Adress.Longitude)) <= model.Distance)
                {
